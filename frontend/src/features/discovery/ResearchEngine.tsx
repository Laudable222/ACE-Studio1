  ...
  return <div className="dx-split" style={{flex:1,minHeight:0}}>
    ...
    <div className="panel" style={{padding:14,overflow:"auto"}}>
      ...
      {!analysis?<div className="empty">Analyse a report to see its findings and hypotheses.</div>:<>
        ...
        {(analysis.hypotheses||[]).map((h:any,i:number)=><div key={i} style={{padding:"10px 0",borderBottom:"1px solid var(--line)"}}><b>{h.statement||h.idea}</b>{h.evidence&&<span className={h.evidence==="MODEL_INFERENCE"?"badge":"badge ok"} style={{marginLeft:6,fontSize:10}}>{h.evidence==="MODEL_INFERENCE"?"inferred":"stated"}</span>}{h.confidence!=null&&<span className="mut" style={{marginLeft:6,fontSize:10}}>confidence {h.confidence}</span>}<div className="mut" style={{fontSize:11,marginTop:4}}>{h.mechanism||""} · sign {h.expected_sign||h.sign||"?"} · horizon {h.horizon||"?"}{h.source_lines?.length?` · lines ${h.source_lines.join(", ")}`:""}</div><div style={{display:"flex",gap:6,marginTop:7}}><button className="btn sm" onClick={()=>createExperiment(h)}>Create experiment</button><button className="btn ghost sm" onClick={()=>sendToGeneration(h)}>Copy generation brief</button></div></div>)}
