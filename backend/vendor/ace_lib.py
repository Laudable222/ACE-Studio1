...
def generate_alpha(
    ...
    alpha_type: Literal["REGULAR", "SUPER", "PYTHON"] = "REGULAR",
    region: str = "USA",
    ...
    unit_handling: Literal["VERIFY", "OFF"] = "VERIFY",
    ...
    # =========================================================================
    # MODIFIED BLOCK: Added explicitly mapped unit and nan parameters for Python
    # =========================================================================
    if alpha_type == "PYTHON":
        settings["language"] = "PYTHON"
        # PYTHON alphas reject unitHandling=VERIFY and nanHandling=OFF (platform 400).
        # Only the known-bad values are coerced; anything else passes through:
        #   - unitHandling is omitted when VERIFY (units are handled in user code)
        #   - nanHandling falls back to ON when OFF is requested
        if unit_handling != "VERIFY":
            settings["unitHandling"] = unit_handling
        settings["nanHandling"] = nan_handling if nan_handling != "OFF" else "ON"
        ...
        logger.error("alpha_type should be REGULAR, SUPER, or PYTHON")
        return {}
...
def check_session_and_relogin(s: SingleSession) -> SingleSession:
    """
    Studio build: NEVER auto-relogin.

    The original version called start_session() (interactive biometrics) whenever
    the auth check reported fewer than 2000 seconds — but that check returns 0 on
    any transient network hiccup, which produced spurious "session expired mid-run"
    failures near the end of a run. The Studio app manages the session itself via
    session_manager; here we just return the current session unchanged.
    """
    return s
...
def simulate_multi_alpha(
    ...
    _ = [set_alpha_properties(s, x["id"], tags=[tag_to_set]) for x in simulation_result["result"]]
    return result
